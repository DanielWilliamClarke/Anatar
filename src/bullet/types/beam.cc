...
Beam::Beam(BulletTrajectory& trajectory, std::shared_ptr<BulletConfig> config, std::shared_ptr<IRayCaster> rayCaster, sf::FloatRect bounds, float damageRate)
...
std::vector<EntityCollision> Beam::DetectCollisions(std::shared_ptr<QuadTree<std::shared_ptr<Entity>>> quadTree)
{
	...
	for (auto& t : targets)
	{
		auto c = t.data->DetectCollisionWithRay(this->position, this->velocity);
		if (c->intersects) {
			collisions.push_back(EntityCollision(t.data, c->point));
		}
	}
